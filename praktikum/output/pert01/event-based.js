...
function draw() {
  ...
  ctx.fillRect(0, 0, W, H);
  if (trail) ctx.fillStyle = "rgba(7,17,31,.13)";
  ...
  ctx.fillText("Canvas 960 Ã— 600 Â· Event-based translation", 18, 28);
  ctx.fillText(
...
window.addEventListener("keydown", (e) => {
  ...
  keys[e.key] = true;
  keydowns++;
  $("eventText").textContent = keydowns + " keydown";
  ...
  if (e.key === "ArrowRight" || k === "d") player.x += s;
  if (e.key === "ArrowLeft" || k === "a") player.x -= s;
  if (e.key === "ArrowUp" || k === "w") player.y -= s;
  if (e.key === "ArrowDown" || k === "s") player.y += s;
  if (k === "p") togglePause();
  if (k === "r") reset();
  if (k === "c") {
    colorIndex = (colorIndex + 1) % colors.length;
...
// Event repeat tetap menggerakkan player. Aksi diskrit di handler utama
// memakai guard repeat agar pause/reset/color tidak berulang.
window.addEventListener("keydown", (e) => {
  if (!e.repeat) return;
  const k = e.key.toLowerCase(),
    s = Number($("speed").value);
  if (e.key === "ArrowRight" || k === "d") player.x += s;
  if (e.key === "ArrowLeft" || k === "a") player.x -= s;
  if (e.key === "ArrowUp" || k === "w") player.y -= s;
  if (e.key === "ArrowDown" || k === "s") player.y += s;
